...
4. 通过命令行 CMD 配置 Windows 防火墙，阻断 tcp/3389 端口连接的命令是；
   ...
   ```
   C:\Windows\system32>netsh advfirewall firewall add rule name="block 3389" dir=in action=block localp
   ort=3389 protocol=tcp
   确定。
...
# 方法二：如果是XP系统的镜像文件，可以直接使用notepad插件得到f
```
