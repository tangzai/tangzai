...
1. Which user installed TeamViewer?

   ```bash
   # 查看 C://Users/ 查看用户情况
   # 进入用户家目录查看 AppData/Roaming
   
   C://Users/Administrator/AppData/Romaing 	# 在这个文件夹中，Windows 会存储已经运行的应用程序的预取数据。这些数据包括了应用程序的文件和操作，以便在下次启动应用程序时更快地加载它们，从而提高启动速度。通常情况下，您不需要手动管理这个文件夹，因为 Windows 会自动维护其中的数据。
   ```

   ![image-20230912021418198](%E9%87%91%E7%A0%962023.assets/image-20230912021418198.png)

2. How many times was teamviewer_desktop.exe run?

   ```bash
   # C://Windows/Prefetch
   # 功能：这个文件夹主要用于存放系统已访问的文件预读信息1234。预读信息是系统在访问时自动生成的新信息，用于加快文件的访问速度2。每个应用程序都会在 Prefetch 目录中留下相应的预读取文件，预读取文件描述了应用程序或系统启动时各个模块的装载顺序5。
   
   #文件格式：这些预读文件以 *.pf 为扩展名，保存在 C://Windows/Prefetch 文件夹内25。它们包含了载入文件的详细信息和载入顺序5。
   
   #文件命名方式：预读取文件的命名方式是以应用程序的可执行文件名为基础，加上一个 “-” 和描述执行文件完整路径的十六进制值，再加上文件扩展名 PF 构成5。
   
   #	清理：由于有些程序产生的 .pf 文件已经不再使用（如安装删除程序），长时间后该文件夹内会产生过多失效的 .pf 文件，系统过久的索引反而会拖慢系统，所以应对其进行及时清理2。
   ```

   ![image-20230912025916015](%E9%87%91%E7%A0%962023.assets/image-20230912025916015.png)

将该文件导出，然后打开`winprefetch`，直接搜索文件名，如果找不到就将这个文件保存到本地的：`C://Windows/Prefetch`

![image-20230912030044997](%E9%87%91%E7%A0%962023.assets/image-20230912030044997.png)	

3. What is the timezone of the desktop image?

   ```bash
   # 下载注册表文件
   # C://windows/system32/system
   # 使用 Registry Viewer 工具查看
   # 注册表路径：ControlSet001\Control\TimeZoneInformation\TimeZoneKeyName 
   ```

   底下的英文连起来就是

![image-20230912031308348](%E9%87%91%E7%A0%962023.assets/image-20230912031308348.png)

4. What is the IP address of the machine?

   ```bash
   # 跟第三步一样，下载 system 注册表并使用 Registry Viewer 打开，记录路径在ControlSet001\Services\Tcpip\Parameters\Interfaces
   ```

   ![image-20230912031557013](%E9%87%91%E7%A0%962023.assets/image-20230912031557013.png)



5. Which user restarted the machine on February 25, 2019?

```
Windows 计算机重启的事件 ID 主要有以下几种1234：

事件 ID 41：当计算机意外地被关闭或重启时，会生成一个 Kernel power 事件的错误日志（日志 ID 为 41）15。这个事件 ID 可能不包含足够的信息来显式定义所发生的情况。通常，还必须考虑意外关闭时发生的情况 (例如电源故障) 1。

事件 ID 1074：通过这个事件 ID 可以查看计算机的开机、关机、重启的时间以及原因和注释234。

事件 ID 6005：表示计算机日志服务已启动，如果出现了事件 ID 为 6005，则表示这天正常启动了系统234。

事件 ID 6006：表示“事件日志服务已停止”，这与系统关闭同义2。
```

```bash
# 下载该文件 C:\Windows\System32\winevt\Logs\system，使用事件查看器打开
```

![image-20230912032748930](%E9%87%91%E7%A0%962023.assets/image-20230912032748930.png)
...
3. What is the SHA1 hash of the Ignition Casino executable?
   ...
   ```
   官方链接：https://learn.microsoft.com/zh-cn/windows/security/threat-protection/auditing/basic-audit-system-events
   ```
   ...
   2. **事件 ID 4625** - 登录失败
      - 描述：登录尝试失败。
      - 用途：用于检测登录尝试失败，可能是入侵者的痕迹。
   3. **事件 ID 4768** - Kerberos TGT 请求
      - 描述：用于跟踪 Kerberos 颁发的 TGT（票证授予票证）请求。
      - 用途：可用于检测针对 Kerberos 验证的攻击。
   4. **事件 ID 4776** - 帐户已被禁用
      - 描述：用户帐户被禁用。
      - 用途：用于监视帐户状态的变化，例如禁用帐户。
   5. **事件 ID 4688** - 新进程的创建
      - 描述：新进程被创建。
      - 用途：用于跟踪新进程的创建，可以用于检测恶意软件活动。
   6. **事件 ID 7045** - 服务状态更改
      - 描述：Windows 服务的状态已更改。
      - 用途：用于检测服务状态的不正常更改，可能是恶意软件。
   7. **事件 ID 7035** - 服务控制管理器
      - 描述：服务控制管理器的操作。
      - 用途：用于监视服务启动和停止的事件。
   8. **事件 ID 5156** - Windows 高级安全审计
      - 描述：对象访问已记录。
      - 用途：用于监视文件和资源的访问。
   9. **事件 ID 1102** - 安全审计日志已清除
      - 描述：安全审计日志已被清除。
      - 用途：用于检测安全日志被删除的行为。
   10. **事件 ID 7040** - 启动或停止了某项服务
       - 描述：已启动或停止了服务。
       - 用途：用于跟踪服务启动和停止的事件。
   
   - [**事件 ID 1074**：通过这个事件 ID 可以查看计算机的开机、关机、重启的时间以及原因和注释](https://learn.microsoft.com/zh-cn/windows/security/application-security/application-control/windows-defender-application-control/operations/event-id-explanations)[1](https://learn.microsoft.com/zh-cn/windows/security/application-security/application-control/windows-defender-application-control/operations/event-id-explanations)。
   - [**事件 ID 6005**：表示计算机日志服务已启动，如果出现了事件 ID 为 6005，则表示这天正常启动了系统](https://learn.microsoft.com/zh-cn/windows/security/application-security/application-control/windows-defender-application-control/operations/event-id-explanations)[1](https://learn.microsoft.com/zh-cn/windows/security/application-security/application-control/windows-defender-application-control/operations/event-id-explanations)。
   - [**事件 ID 6006**：表示“事件日志服务已停止”，这与系统关闭同义](https://learn.microsoft.com/zh-cn/windows/security/application-security/application-control/windows-defender-application-control/operations/event-id-explanations)[1](https://learn.microsoft.com/zh-cn/windows/security/application-security/application-control/windows-defender-application-control/operations/event-id-explanations)。
   - [**事件 ID 1100**：表示“事件记录服务已关闭”](https://zhuanlan.zhihu.com/p/528734598)[2](https://zhuanlan.zhihu.com/p/528734598)。
   - [**事件 ID 1101**：表示“审计事件已被运输中断”](https://zhuanlan.zhihu.com/p/528734598)[2](https://zhuanlan.zhihu.com/p/528734598)。
   - [**事件 ID 1102**：表示“审核日志已清除”](https://zhuanlan.zhihu.com/p/528734598)[2](https://zhuanlan.zhihu.com/p/528734598).
   - [**事件 ID 4624**：表示“帐户已成功登录”](https://zhuanlan.zhihu.com/p/528734598)[2](https://zhuanlan.zhihu.com/p/528734598).
   - [**事件 ID 4625**：表示“帐户无法登录”](https://zhuanlan.zhihu.com/p/528734598)[2](https://zhuanlan.zhihu.com/p/528734598).
   - [**事件 ID 4634**：表示“帐户已注销”](https://zhuanlan.zhihu.com/p/528734598)[2](https://zhuanlan.zhihu.com/p/528734598).
   - [**事件 ID 4647**：表示“用户启动了注销”](https://zhuanlan.zhihu.com/p/528734598)[2](https://zhuanlan.zhihu.com/p/528734598).
   - [**事件 ID 4672**：表示“分配给新登录的特权”](https://zhuanlan.zhihu.com/p/528734598)[2](https://zhuanlan.zhihu.com/p/528734598).
   - [**事件 ID 4688**：表示“已经创建了一个新流程”](https://zhuanlan.zhihu.com/p/528734598)[2](https://zhuanlan.zhihu.com/p/528734598).
   - [**事件 ID 4689**：表示“一个过程已经退出”](https://www.bing.com/search?form=MY02AA&OCID=MY02AA&pl=launch&q=Bing+AI&showconv=1#)[2](https://zhuanlan.zhihu.com/p/528734598).

```
$Recycle.Bin：回收站，用于保存删除文件数据。可以删除，会自动重建。
Boot：用于保存操作系统启动引导文件的文件夹。绝对不能删除。
Documents and Settings：系统用户设置文件夹，包括各个用户的文档、收藏夹、上网浏览信息、配置文件等。不能删除，否则会造成系统重新启动不能登陆的情况发生。
EFI：自己装的系统可能会有这个文件夹，里面的是启动文件，用于GPT分区，UEFI启动的机器。可以删除。
PerfLogs：是系统日志信息文件夹，如磁盘扫描错误信息，是系统自动生成的。不建议删除，会降低系统速度。
Program Files：系统自带应用应用和软件的安装目录。不能删除。
Program Files (X86)：这是系统中32位软件的安装目录。不能删除。
Program Data：Wind7以上系统系统文件夹，放置程序的使用数据、设置等文件。不建议删除。
Recovery：用于存放恢复系统用的恢复环境的文件夹。可以删除这个文件夹，对系统不会造成任何影响。
System Volume Information：这是任何一个分区下都会存在的一个文件夹，一般翻译成系统卷标信息文件夹。用于存储系统还原的备份信息以及磁盘分区数据信息。可以删除。
Windows：是Windows系统安装文件所在文件夹，一般用来存储系统安装文件和硬件驱动程序等内容，是系统正常运转的必要保证。不能删除。
Users：用来储存系统所有用户数据、用户应用程序数据、桌面和开始菜单等内容的文件夹。一般不删除，但可以移动到其他分区。
```
...
└─# nmap -T4 -sV -p- -A -Pn -O 192.168.56.101
Starting Nmap 7.92 ( https://nmap.org ) at 2023-09-12 16:48 CST
Nmap scan report for 192.168.56.101
Host is up (0.0011s latency).
Not shown: 65530 closed tcp ports (reset)
PORT      STATE SERVICE            VERSION
2121/tcp  open  ftp                ProFTPD
4224/tcp  open  ssh                OpenSSH 8.9p1 Ubuntu 3ubuntu0.3 (Ubuntu Linux; protocol 2.0)
| ssh-hostkey: 
|   256 29:93:3f:43:fe:e0:5e:19:77:8f:c7:10:96:10:af:02 (ECDSA)
|_  256 3f:16:22:f0:e0:47:95:f0:4e:fa:10:c4:c2:13:fd:10 (ED25519)
8800/tcp  open  http               Apache httpd 2.4.52 ((Ubuntu))
|_http-server-header: Apache/2.4.52 (Ubuntu)
|_http-title: Home | Simple PHP Website
| http-robots.txt: 2 disallowed entries 
|_index_old.php login.php
31337/tcp open  hadoop-tasktracker Apache Hadoop 2.4.38 ((Debian))
...
MAC Address: 08:00:27:BC:85:C7 (Oracle VirtualBox virtual NIC)
Device type: general purpose
Running: Linux 4.X|5.X
OS CPE: cpe:/o:linux:linux_kernel:4 cpe:/o:linux:linux_kernel:5
OS details: Linux 4.15 - 5.6
