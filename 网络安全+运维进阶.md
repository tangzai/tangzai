...
1. 先查看`access.log`日志并过滤出200状态码的日志信息
2. 分析这些日志信息查看是否存在SQL注入漏洞、弱口令爆破等
...
```shell
[root@localhost 日志文件分析]# awk '$9~/^200$/ {print $0}' access.log |sort -nr > 200.txt
```

+ `$9~/^200$/`：正则匹配状态码为200的信息条目
+ `{print $0}`：打印出整条日志信息
+ `sort -nr`：-n是按照数字进行排序，-r是反向排序，以保持文档原有的状态

**2. 过滤出敏感路径**

```shell
[root@localhost 日志文件分析]#  awk '$7~ /\/cas\/login/ {print $0}' 200.txt |sort -nr |head
```

+ `$7~ /\/cas\/login/`：匹配`/cas/login`的访问路径信息

**3. 提取不是200的日志**

```shell
[root@localhost 日志文件分析]#  awk '$9~!/^200$/ {print $0}' access.log > no200.txt
```
...
```shell
[root@localhost 日志文件分析]# awk '$7~/^\/index.php\?q=category&search=Banking%27%20/ {print $0}' access.log |wc -l
```
...
```shell
[root@localhost 日志文件分析]# awk '{print $1}' |sort|uniq -c
```

+ `sort`：比较原则是从首字符向后依次按ASCII码值进行比较，最后将他们按升序输出。
+ `uniq -c`： 在每行开头增加重复次数。
...
+ 警告事件：指不是直接的、主要的，但会导致将来问题发生的事件
+ 错误事件：值用户应该知道的重要问题。通常是功能和数据的丢失。如一个服务不能作为系统引导被加载，就会产生一个错误事件
+ 成功审核（Success audit）：表示**成功的审核**安全访问尝试，主要是指安全日志，包含用户的登录\注销，对象访问、特权使用、账户管理、策略更改、详细跟踪、目录服务访问、账户登录等事件，其中所有成功登录的系统都会被记录为“成功审核”事件
+ 失败审核（Failure aduit）：表示**失败的审核**安全登录尝试，如用户试图访问网络驱动器失败

#### 3.2 Windows 日志文件

+ 系统日志（SysEvent）：记录操作系统组件产生的事件。主要包括驱动程序、系统组件和应用软件的崩溃及数据
+ 应用程序日志（AppEvent）：包含由应用程序或系统程序记录的事件，主要记录程序运行方面的事件
+ 安全日志（SecEvent）：记录系统的**安全审计事件**，包含各类系统的登录日志、对象访问日志、进程追踪日志、特权使用日志、账号管理、策略变更、系统事件。安全日志也是调查曲中中最常用到的
...
| 登录类型 |              描述              |                     说明                     |
| :------: | :----------------------------: | :------------------------------------------: |
|    2     |    交互登录（Interactive）     |              用户在本地进行登录              |
|    3     |        网络（Network）         |  最常见的情况是链接到共享文件夹或共享打印机  |
|    4     |        批处理（Batch）         |            通常表明某计划任务启动            |
|    5     |        服务（Service）         |  每种服务都被配置在某个特定的用户账号下运行  |
|    7     |         解锁（Unlock）         |                   屏幕解锁                   |
|    8     | 网络明文（Network Cleartext）  |  登录的命名在网络上是通过明文传输的，如FTP   |
|    9     |   新凭证（New Credentials）    | 使用带/Netonly 参数的 RUNAS 命令运行一个程序 |
...
> Log Parser Lizard 是一款强大的GUI图形界面版的日志分析工具，且支持SQL语句做查询
>
> 参考链接：https://blog.51cto.com/hzc2012/6001412
>
> 奇安信-网络安全-应急相应技术实践：P52

### 4. Linux 操作系统日志

#### 4.1 Linux 日志概述

`/etc/rsyslog.conf`记录各种系统服务的日志文件路径

![image-20240215171254356](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240215171254356.png)

Linux 系统内核和许多程序都会产生各种错误信息、警告信息和其他的提示信息，这些信息对管理员了解系统的运行状态是非常有用的，所以应该把它们写道日志文件中去，完成这个过程的程序就是`syslog`。syslog可以根据日志的类别和优先级将体制保存到不同的文件中，日志的优先级越小越高
...
| 5    | NOTICE   | 注意     | 不会影响正常功能，但系需要注意的事件       |
| 6    | INFO     | 信息     | 一般信息                                   |
...
| /var/log/lastlog | 记录系统所有用户最后一次登录时间的日志，这个文件是二进制的，不能直接使用vi查看，而要使用lastlog命令 |
| /var/log/wtmp    | 记录当前已经登录的用户信息，会随着用户的登录和注销的不断变化，只记录当前登录用户的信息。这个文件不能直接使用vi查看，而要使用w、who、users等命令 |
| /var/log/secure  | 记录严重和授权方面的信息，只要设计账号和密码的程序都会记录，如SSH登录、su切换用户、sudo授权、以及添加用户和修改用户密码都会记录在这个日志文件中 |

#### 4.2 Linux重要日志文件的解析

**4.2.1 `/var/log/uamp`**

该日志文件用于记录当前登录的用户信息，使用`w`命令、`who`命令、`users`命令进行查看

<img src="%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240215172857245.png" alt="image-20240215172857245" style="zoom:67%;" />

+ `w`命令：查询`utmp`文件，并显示当前系统中每个用户和它所运行的进程信息
+ `who`命令：查询utmp文件，并报告当前登录的每个用户
+ `users`命令：用单独的一行打印出当前登录的用户，每个显示的用户对应一个登录会话

**4.2.2 `/var/log/lastlog`**

显示最后一次**登录成功**的用户，可使用`lastlog`命令查看

<img src="%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240215173118736.png" alt="image-20240215173118736" style="zoom: 80%;" />

**4.2.3 `/var/log/wtmp`**

永久记录每个用户登录、注销及系统的启动、停机事件。可使用`last`命令查看

<img src="%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240215173213497.png" alt="image-20240215173213497" style="zoom:80%;" />

**4.2.4 `/var/log/btmp`**

记录Linux登录失败的用户、事件及远程IP地址，可直接使用`lastb`命令查看

![image-20240215173335880](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240215173335880.png)

**4.2.5 `/var/log/secure`**

用于记录用户登录认证的相关日志，SSH服务的登录信息也被记录到这里

<img src="%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240215173514699.png" alt="image-20240215173514699" style="zoom:80%;" />

**4.2.6 `/var/log/yum.log`**软件安装日志

#### 4.3 Linux 日志的分析思路

**4.3.1 查找有哪些IP地址再做暴力破解**

```bash
[root@localhost etc]# grep "Failed password" /var/log/secure |awk '{print $11}' |sort |uniq -c |sort -nr
```
...
```bash
[root@localhost etc]# grep "Failed password" /var/log/secure |awk '{print $9}' |sort |uniq -c |sort -nr
     60 root
```
...
```bash
[root@localhost etc]# grep "Accepted" /var/log/secure-20240215 |awk '{print $9,$11}' |sort |uniq -c |sort -nr
     12 root 192.168.231.1
      1 root 192.168.231.129
```
...
```
ip.addr===10.2.9.4				# 显示源目IP的地址是10.2.9.4的流量
!ip.addr===10.9.2.4				# 显示源目IP地址不是10.2.9.4的流量
ip.src===10.9.2.4				# 显示源地址是10.2.9.4的流量
ip.dst=10.9.2.4					# 显示目标地址是10.2.9.4的流量
ip.hpst===www.baidu.com			# 显示到达或来自解析www.baidu.com网站后的IP地址数据
```

**2. 筛选一个地址范围的数据**

```
ip.src > 10.2.7.2 && ip.src < 10.2.7.5								# 显示IP源地址是 10.2.7.3、10.2.7.4 的数据
(ip.src > 10.2.7.2 && ip.src < 10.2.7.5) && !ip.src==10.2.7.3		# 只显示 10.2.7.4 的数据
ip.src===10.2.7.0/24												# 显示IP源地址在 10.2.7.0/24 网段的数据
```
...
```
tcp								# 显示所有基于TCO的流量
tcp.port===80					# 显示所有TCP 80 端口的流量
(ip.addr eq 10.2.9.4 and ip.addr eq 203.119.114.20) and (tcp.port eq 50248 and tcp.port eq 80)		# eq 直接理解为等号
bootp							# 筛选DHCP流量
http.request.method===POST		# 显示HTTP POST方法的流量
```

**4. 数据统计**

1）单个IP端点数据发送或接收的统计：选择 “统计” ---> “端点”

![image-20240218153351082](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218153351082.png)

2）两个IP间会话的统计：选择 “统计” ---> "会话"

![image-20240218153432413](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218153432413.png)

**5. 协议分层统计**

选择 “统计” ---> “协议分级”

![image-20240218153519216](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218153519216.png)

**6. 显示带宽数据**

选择：“统计” ---> "I/O 图表"

![image-20240218153559588](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218153559588.png)

**7. 从流量中还原文件**

对于HTTP流量，是可以直接从源数据的方式将文件复制下来并重新写入的，定位数据的开头则可以使用文件头来定位，以下是常见的文件头：
...
+ `http.host===baidu.com`：过滤经过指定域名的HTTP数据包
...
+ `http.request.uri==="/online/admin`：过滤请求的URI，取值是域名的后的部分
+ `http.request,full_uri===http://www.baidu.com`：过滤含有域名的整个URL
+ `http.server contains "nginx"`：过滤HTTP头中Server字段含有“nginx”字段的数据包
+ `http.content_type==="text/html"`：过滤content_type是“text/html”的HTTP数据包
+ `http.content_encoding===gzip`：过滤content_encoding是“gzip”的HTTP包
+ `http.content_length`：根据content_length的数值进行过滤
+ `http.server`：过滤所有HTTP头含有server字段的数据包
+ `http.request.version==="HTTP/1.1"`：过滤HTTP/1.1版本的HTTP包，包括请求包和相应包
...
![image-20240218164938480](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218164938480.png)

2. ## 排查可疑用户

![image-20240218164959670](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218164959670.png)

## 3.排查历史命令

![image-20240218165022156](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218165022156.png)

![image-20240218165058307](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218165058307.png)

## 4. 排查可疑文件

**1）敏感目录文件**

![image-20240218165122751](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218165122751.png)

**2）查找新增文件**

查找24小时内被修改的PHP文件

![image-20240218165207336](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218165207336.png)

## 5. 排查开机自启项

下图中的0-6代表的是运行级别

![image-20240218165324982](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218165324982.png)
...
![image-20240218165539817](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218165539817.png)

# Windows 终端检测

## 1. 网络链接检测

![image-20240218233840067](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218233840067.png)

## 2. 排查可疑用户

![image-20240218233917573](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218233917573.png)

**命令 `net user` 无法查看隐藏用户**

## 3. 排查可疑文件

+ 桌面文件：`C:\User\hack\Desktop`
+ 用户网络访问情况：`C:\User\hack\Cookies`文件中可能会记录一些敏感信息
+ 用户最近访问过哪些文件或文件夹：`C:\User\hack\Recent`
+ 用户上网的历史记录：`C:\User\hack\Local Settings\History`
+ 一些程序安装、解压缩等操作可能会在该目录产生临时文件：`C:\User\hack\Local Seetings\Temp`
+ 下载文件：`C:\User\hack\Downloads`

## 4. 排查开机启动项

1. `HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run`：这个键用于设置当前用户登录时需要启动的程序。你可以在这个键下新建一个字符串值，键名为你自己能记住的名称，键值为需要启动的程序的绝对路径
2. `HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Run`：这个键用于设置所有用户登录时需要启动的程序。同样，你可以在这个键下新建一个字符串值，键名为你自己能记住的名称，键值为需要启动的程序的绝对路径
3. `HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce`

## 5. 排查计划任务

![image-20240218234727311](%E7%BD%91%E7%BB%9C%E5%AE%89%E5%85%A8+%E8%BF%90%E7%BB%B4%E8%BF%9B%E9%98%B6.assets/image-20240218234727311.png)
