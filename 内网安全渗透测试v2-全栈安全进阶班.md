...
工作组中的计算机都是平等的，在同一工作组的计算机可以在网络上相互发现。但是工作组无法做到使用某一台计算机控制所有计算机这一操作，在中大型的网络中，维护起来会非常麻烦

## 1-4 域

### 域环境基础概念

+ 组织单元OU：组织单元（Organizational Unit，OU）是Windows域中的一个容器，可以将用户、组、计算机和其他组织单位放入其中。它是可以指派组策略设置或委派管理权限的最小作用域或单元
+ 安全标识符SID：相当于计算机的身份证，用于标识该计算机的所有权限（s-1-5-21-D1-D2-D3-RID）
