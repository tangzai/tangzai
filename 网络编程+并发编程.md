...
if __name__ == '__main__':
    p = Process(target=func)		# 实例化一个即将执行func函数的对象
    p.start()						# 执行func函数
...
从输出结果可以看到，代码的执行是先执行'main'，再执行函数“func”，这是因为python在创建了一个新进程“p”之后并没有执行，而是在`p.start()`的时候才开始执行，并且期间并不关系进程是否运行正常而去继续执行下一条代码。所以最后结果是先执行“main”再执行函数“func”
...
+ Windows系统：子进程会导入父进程的代码并重新运行计算。注意使用main预防进程的死循环
+ Linux系统：子进程会直接导入父进程的代码的运行状态，继续执行，其中不用重新计算

父进程的结束：夫进程的结束需要满足以下几点：
...
> 由于python的劣势，一帮来讲python做能做单进程单线程的操作或者多进程单线程的操作
